...
class Webform extends EntityReferenceBase implements ContainerFactoryPluginInterface {
  ...
  public function viewValue(FieldItemInterface $item, $delta, array $contexts) {
    ...
    if ($this->isLayoutBuilder($contexts)) {
      $element = $this->getPlaceholder($webform);
    }
    else {
   ...
   *   A renderable element.
   */
  protected function getPlaceholder(WebformInterface $webform = NULL) {
    $webform = $webform ?: $this->getDefaultWebform();
    if ($webform) {
      $render = $this->getFormAsPlaceholder($this->getWebformRenderable($webform));
    }
    else {
      $render = $this->componentPlaceholderDefault('Form');
    }
    return $render;
  }
   ...
   * Get default webform.
   *
   * Will return the first found webform.
   *
   * @return \Drupal\webform\WebformInterface|null
   *   The webform.
   */
  protected function getDefaultWebform() {
    $entity = $this->getPreviewEntity('webform');
    return $entity ?: NULL;
  }
